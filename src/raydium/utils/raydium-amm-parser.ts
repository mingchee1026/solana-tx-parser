...
export class RaydiumAmmParser {
  ...
  parseRaydiumSwapIn1(instructionData: Buffer, programId: PublicKey) {
    const args = SwapBaseInLayout.decode(instructionData);
    return {
      name: "swapIn",
      args: {
  ...
  parseRaydiumSwapOut1(instructionData: Buffer, programId: PublicKey) {
    const args = SwapBaseOutLayout.decode(instructionData);
    return {
      name: "swapOut",
      args: {
  ...
  private parseUnknownInstruction(instruction: TransactionInstruction) {
    return {
      name: "Unknown",
      args: { unknown: base58.default.encode(instruction.data) },
      programId: instruction.programId,
    };
  }
